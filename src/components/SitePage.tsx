...
export function SitePage({ crawlId, websiteId, domain, crawl, onBack }: SitePageProps) {
  ...
  const [noindexFilter, setNoindexFilter] = useState(false);
  ...
  useEffect(() => setOffset(0), [crawlId, applied, noindexFilter]);
  ...
  useEffect(() => {
    ...
    setError(null);
    Promise.all([
      getCrawlPages(crawlId, { limit: pageSize, offset, search: applied || undefined, noindex: noindexFilter || undefined }),
  ...
  }, [crawlId, pageSize, offset, applied, noindexFilter]);
              ...
              {!loading && !error && pages.length > 0 && (
                <>
                  <div className="panel-summary">
                    ...
                    >
                      <label style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "13px", cursor: "pointer", color: "var(--fg-muted)" }}>
                        <input
                          type="checkbox"
                          checked={noindexFilter}
                          onChange={(e) => setNoindexFilter(e.target.checked)}
                        />
                        noindex only
                      </label>
                      <input
