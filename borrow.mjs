...
const uid=(await c.query(`INSERT INTO users (email,password_hash) VALUES ('gscidx@example.test',$1) RETURNING id`,[await hashPassword("GscIdx-Passw0rd-1")])).rows[0].id;
const site=(await c.query(`SELECT w.id,w.user_id,w.domain FROM websites w JOIN gsc_properties p ON p.website_id=w.id WHERE p.site_url LIKE '%atmhtml5%' LIMIT 1`)).rows[0];
